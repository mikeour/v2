...
<InteractiveArticleExample caption="Scroll the article — notice how shadows appear and disappear at the edges." />
...
<ArticleProgress caption="Scroll the article to see the value update in real-time." />
...
<ArticleMarkupExample caption="Drag the slider to see how shadow size affects the layout." />
...
For those not using Tailwind, here's the equivalent CSS:
...
Note: This CSS handles positioning only. You'll still need JavaScript to control the opacity based on scroll position — the `useScrollShadows` hook works the same way regardless of how you style the elements.
